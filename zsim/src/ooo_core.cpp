...
inline void OOOCore::bbl(Address bblAddr, BblInfo* bblInfo) {
    ...
    if (branchPc && !branchPred.predict(branchPc, branchTaken)) {
        ...
        for (uint32_t i = 0; i < 5*64/lineSize; i++) {
            //NOT FINSIHED: need to get pc here, wrongPathAddr + lineSize*i is incorrect
            uint64_t fetchLat = l1i->load(wrongPathAddr + lineSize*i, curCycle, wrongPathAddr + lineSize*i) - curCycle; 
    ...
    for (Address fetchAddr = bblAddr; fetchAddr < endAddr; fetchAddr += lineSize) {
        ...
        //NOT FINSIHED: need to get pc here, fetchAddr is incorrect
        uint64_t fetchLat = l1i->load(fetchAddr, curCycle, fetchAddr) - curCycle;
        cRec.record(curCycle, curCycle, curCycle + fetchLat);
        fetchCycle += fetchLat;
    }
